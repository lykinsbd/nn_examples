...
def test_single_transform(target_ip, transform_set, aggressive_mode=False, timeout=5):
    ...
    if result:
        if result["accepted"]:
            print("[+] ACCEPTED - Transform set accepted by peer")
            print(f"    Responder Cookie: {result['responder_cookie']}")
            return True
        else:
            print("[-] REJECTED - No SA payload in response")
            return False
    else:
        print("[-] NO RESPONSE - Timeout or no ISAKMP response")
        return False
...
Examples:
  ...
  %(prog)s 192.168.1.1 --aggressive --timeout 10
    ...
    parser.add_argument("--test-multiple", action="store_true", help="Test multiple common transform sets")
    parser.add_argument("--aggressive", action="store_true", help="Use Aggressive Mode instead of Main Mode")
    parser.add_argument("--timeout", type=int, default=5, help="Response timeout in seconds (default: 5)")
    parser.add_argument(
        "--encryption",
        type=int,
        default=ENCR_AES_CBC,
        help=f"Encryption algorithm (default: {ENCR_AES_CBC} for AES-CBC)",
    )
    parser.add_argument("--key-length", type=int, default=256, help="Key length in bits (default: 256)")
    parser.add_argument(
        "--hash", type=int, default=HASH_SHA256, help=f"Hash algorithm (default: {HASH_SHA256} for SHA-256)"
    )
    parser.add_argument(
        "--dh-group", type=int, default=DH_GROUP_14, help=f"DH group (default: {DH_GROUP_14} for 2048-bit)"
    )
    parser.add_argument("--lifetime", type=int, default=86400, help="SA lifetime in seconds (default: 86400)")
    ...
    try:
        if args.test_multiple:
            ...
            test_single_transform(args.target, transform_set, args.aggressive, args.timeout)
