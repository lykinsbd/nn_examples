...
def build_response(request):
    ...
    try:
        ...
        mode = "Main Mode" if req_isakmp.exch_type == 2 else "Aggressive Mode"
        print(f"    Exchange Type: {req_isakmp.exch_type} ({mode})")

        # Check if SA payload is present
        if not request.haslayer(ISAKMP_payload_SA):
            print("[!] No SA payload found")
            return None
        ...
        sa = ISAKMP_payload_SA(doi=1, situation=1)
        ...
        cookie_display = resp_cookie_val.hex() if isinstance(resp_cookie_val, bytes) else resp_cookie_val
        print(f"[+] Response built, Responder Cookie: {cookie_display}")
...
def start_listener(interface="0.0.0.0", port=500):
    ...
    """
    print("[*] Starting ISAKMP test listener")
    print(f"    Interface: {interface}")
    print(f"    Port: UDP/{port}")
    print("[*] Waiting for ISAKMP packets... (Ctrl+C to stop)\n")
    ...
    def packet_handler(pkt):
        ...
        if pkt.haslayer(ISAKMP):
            ...
            if response:
                ...
                print("[!] No response built")

    # Sniff for ISAKMP packets
    try:
        sniff(filter=f"udp and port {port}", prn=packet_handler, store=0)
...
Note: This is a test tool only. It does NOT implement a real VPN.
    ...
    parser.add_argument("--port", type=int, default=500, help="UDP port to listen on (default: 500)")
